...
put topirler is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: pirl should be started in `pirl --external_signer localhost:8550`.
    - Currently, the pirl APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This
  ...
  retains the original input.
    - The pirl api should switch to use the same type, and relay `to`-account verbatim to the external api.
...
The signer listens to HTTP requests on `rpcaddr`:`rpcport`, with the same JSONRPC standard as pirl. The messages are
