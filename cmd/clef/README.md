Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for pirl's account management.
This allows DApps not to depend on pirl's account management. When a DApp wants to sign data it can send the data to
...
The general flow for signing a transaction using e.g. pirl is as follows:
![image](sign_flow.png)

In this case, `pirl` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
put topirler is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Ethereum should be started in `pirl --external_signer localhost:8550`.
    - Currently, the Ethereum APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This
  ...
  retains the original input.
    - The Ethereum api should switch to use the same type, and relay `to`-account verbatim to the external api.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus pirl/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
The signer listens to HTTP requests on `rpcaddr`:`rpcport`, with the same JSONRPC standard as Ethereum. The messages are
