...
#
# The cluster contains a bootnode, a pirl node and multiple swarm nodes, with
...
# static pirl configuration
pirl_IP="192.168.33.3"
pirl_RPC_PORT="8545"
pirl_RPC_URL="http://${pirl_IP}:${pirl_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the pirl and
# swarm nodes
start_bootnode() {
...
# start_pirl_node starts a pirl node with --datadir pointing at <base-dir>/pirl
# and a single, unlocked account with password "pirl"
start_pirl_node() {
  local dir="${base_dir}/pirl"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get pirl account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${pirl_IP}"
    --rpcport   "${pirl_RPC_PORT}"
    --verbosity "6"
  )

  start_node "pirl" "${pirl_IP}" "$(which pirl)" ${args[@]}
}
...
create_account() {
  ...
  pirl --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
