...
func (f *lightFetcher) checkAnnouncedHeaders(fp *fetcherPeerInfo, headers []*types.Header, tds []*big.Int) bool {
	...
	for i := len(headers) - 1; ; i-- {
		if i < 0 {
			...
			header = f.chain.pirleader(hash, number)
...
func (f *lightFetcher) checkSyncedHeaders(p *peer) {
	...
	header := f.chain.pirleader(n.hash, n.number)
...
func (f *lightFetcher) lastTrustedTreeNode(p *peer) (*types.Header, []common.Hash) {
	...
	if canonical.Number.Uint64() > f.lastTrustedHeader.Number.Uint64() {
		canonical = f.chain.pirleaderByNumber(f.lastTrustedHeader.Number.Uint64())
	}
	...
	for current.Hash() == commonAncestor.Hash() {
		...
		current = f.chain.pirleader(current.ParentHash, current.Number.Uint64()-1)
	}
	return current, unapprovedHashes
}
...
// adds it to the end of a linked list topirler with the time it has been confirmed. Then checks which peers have
