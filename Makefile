...
.PHONY: pirl android ios pirl-cross evm all test clean
...
pirl:
	$(GORUN) build/ci.go install ./cmd/pirl
	@echo "Done building."
...
pirl-linux-386:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/386 -v ./cmd/pirl
	@echo "Linux 386 cross compilation done:"
	@ls -ld $(GOBIN)/pirl-linux-* | grep 386

pirl-linux-amd64:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/amd64 -v ./cmd/pirl
...
pirl-linux-arm-5:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/arm-5 -v ./cmd/pirl
	@echo "Linux ARMv5 cross compilation done:"
	@ls -ld $(GOBIN)/pirl-linux-* | grep arm-5

pirl-linux-arm-6:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/arm-6 -v ./cmd/pirl
	@echo "Linux ARMv6 cross compilation done:"
	@ls -ld $(GOBIN)/pirl-linux-* | grep arm-6

pirl-linux-arm-7:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/arm-7 -v ./cmd/pirl
	@echo "Linux ARMv7 cross compilation done:"
	@ls -ld $(GOBIN)/pirl-linux-* | grep arm-7

pirl-linux-arm64:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/arm64 -v ./cmd/pirl
	@echo "Linux ARM64 cross compilation done:"
	@ls -ld $(GOBIN)/pirl-linux-* | grep arm64

pirl-linux-mips:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/mips --ldflags '-extldflags "-static"' -v ./cmd/pirl
	@echo "Linux MIPS cross compilation done:"
	@ls -ld $(GOBIN)/pirl-linux-* | grep mips

pirl-linux-mipsle:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/mipsle --ldflags '-extldflags "-static"' -v ./cmd/pirl
	@echo "Linux MIPSle cross compilation done:"
	@ls -ld $(GOBIN)/pirl-linux-* | grep mipsle

pirl-linux-mips64:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/mips64 --ldflags '-extldflags "-static"' -v ./cmd/pirl
	@echo "Linux MIPS64 cross compilation done:"
	@ls -ld $(GOBIN)/pirl-linux-* | grep mips64

pirl-linux-mips64le:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/mips64le --ldflags '-extldflags "-static"' -v ./cmd/pirl
...
pirl-darwin-386:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=darwin/386 -v ./cmd/pirl
	@echo "Darwin 386 cross compilation done:"
	@ls -ld $(GOBIN)/pirl-darwin-* | grep 386

pirl-darwin-amd64:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=darwin/amd64 -v ./cmd/pirl
...
pirl-windows-386:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=windows/386 -v ./cmd/pirl
	@echo "Windows 386 cross compilation done:"
	@ls -ld $(GOBIN)/pirl-windows-* | grep 386

pirl-windows-amd64:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=windows/amd64 -v ./cmd/pirl
